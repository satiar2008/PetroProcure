...
internal sealed class AiEvaluationJobConfiguration : IEntityTypeConfiguration<AiEvaluationJob> { public void Configure(Microsoft.EntityFrameworkCore.Metadata.Builders.EntityTypeBuilder<AiEvaluationJob> b) { b.ToTable("AiEvaluationJobs", DatabaseSchemas.Ai); b.ConfigureEntity(); b.Property(x => x.EvaluationType).HasMaxLength(100); b.HasIndex(x => x.PurchaseFileId); } }
internal sealed class AiEvaluationResultConfiguration : IEntityTypeConfiguration<AiEvaluationResult> { public void Configure(Microsoft.EntityFrameworkCore.Metadata.Builders.EntityTypeBuilder<AiEvaluationResult> b) { b.ToTable("AiEvaluationResults", DatabaseSchemas.Ai); b.ConfigureEntity(); b.Property(x => x.EvaluationType).HasMaxLength(100); b.Property(x => x.Summary).HasMaxLength(8000); b.HasMany(x => x.Findings).WithOne().HasForeignKey(x => x.EvaluationResultId); b.HasMany(x => x.Recommendations).WithOne().HasForeignKey(x => x.EvaluationResultId); b.Navigation(x => x.Findings).UsePropertyAccessMode(PropertyAccessMode.Field); b.Navigation(x => x.Recommendations).UsePropertyAccessMode(PropertyAccessMode.Field); } }
internal sealed class AiFindingConfiguration : IEntityTypeConfiguration<AiFinding> { public void Configure(Microsoft.EntityFrameworkCore.Metadata.Builders.EntityTypeBuilder<AiFinding> b) { b.ToTable("AiFindings", DatabaseSchemas.Ai); b.ConfigureEntity(); b.Property(x => x.Title).HasMaxLength(500); b.Property(x => x.Description).HasMaxLength(4000); b.Property(x => x.Code).HasMaxLength(200); } }
