...
public static class InquiryEndpoints
{
    public static IEndpointRouteBuilder MapInquiryEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        inquiries.MapPost("/", async (CreateInquiryRequest r, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => Results.Created("/api/inquiries", await h.Handle(new CreateInquiryCommand(r.PurchaseFileId, r.Title, r.InquiryType, r.DeadlineDate, r.Description), ct))))
        ...
        inquiries.MapPost("/from-purchase-file/{purchaseFileId:guid}", async (Guid purchaseFileId, CreateInquiryFromPurchaseFileRequest r, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => Results.Created("/api/inquiries", await h.Handle(new CreateInquiryFromPurchaseFileCommand(purchaseFileId, r.Title, r.InquiryType, r.DeadlineDate, r.Description, r.PurchaseFileItemIds, r.SupplierIds), ct))))
            .RequirePermission(ApplicationPermissions.InquiryCreate);
        inquiries.MapPut("/{id:guid}", async (Guid id, UpdateInquiryRequest r, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => Results.Ok(await h.Handle(new UpdateInquiryCommand(id, r.Title, r.InquiryType, r.DeadlineDate, r.Description), ct))))
        ...
        inquiries.MapPost("/{id:guid}/send", async (Guid id, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => { await h.Handle(new SendInquiryCommand(id), ct); return Results.NoContent(); })).RequirePermission(ApplicationPermissions.InquirySend);
        inquiries.MapPost("/{id:guid}/cancel", async (Guid id, CancelInquiryRequest r, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => { await h.Handle(new CancelInquiryCommand(id, r.Reason), ct); return Results.NoContent(); })).RequirePermission(ApplicationPermissions.InquiryCancel);
        inquiries.MapPost("/{id:guid}/close", async (Guid id, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => { await h.Handle(new CloseInquiryCommand(id), ct); return Results.NoContent(); })).RequirePermission(ApplicationPermissions.InquiryEdit);
        ...
        inquiries.MapPost("/{id:guid}/items", async (Guid id, AddInquiryItemRequest r, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => Results.Created($"/api/inquiries/{id}/items", await h.Handle(new AddInquiryItemCommand(id, r.PurchaseFileItemId), ct))))
            .RequirePermission(ApplicationPermissions.InquiryEdit);
        inquiries.MapDelete("/{id:guid}/items/{itemId:guid}", async (Guid id, Guid itemId, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => { await h.Handle(new RemoveInquiryItemCommand(id, itemId), ct); return Results.NoContent(); })).RequirePermission(ApplicationPermissions.InquiryEdit);
        ...
        inquiries.MapPost("/{id:guid}/suppliers", async (Guid id, AddInquirySupplierRequest r, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => Results.Created($"/api/inquiries/{id}/suppliers", await h.Handle(new AddInquirySupplierCommand(id, r.SupplierId, r.ContactId), ct))))
        ...
        inquiries.MapDelete("/{id:guid}/suppliers/{inquirySupplierId:guid}", async (Guid id, Guid inquirySupplierId, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => { await h.Handle(new RemoveInquirySupplierCommand(id, inquirySupplierId), ct); return Results.NoContent(); })).RequirePermission(ApplicationPermissions.InquiryManageSuppliers);
        ...
        inquiries.MapPost("/{id:guid}/quotes", async (Guid id, AddSupplierQuoteRequest r, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => Results.Created($"/api/inquiries/{id}/quotes", await h.Handle(new AddSupplierQuoteCommand(id, r.InquirySupplierId, r.QuoteNumber, r.QuoteDate, r.ValidUntil, r.Currency, r.DeliveryTerms, r.PaymentTerms, r.DeliveryDate, r.TotalAmount, r.TaxAmount, r.DiscountAmount, r.TechnicalNote, r.CommercialNote), ct))))
        ...
        inquiries.MapPost("/{id:guid}/quotes/{quoteId:guid}/items", async (Guid id, Guid quoteId, AddSupplierQuoteItemRequest r, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => Results.Created($"/api/inquiries/{id}/quotes/{quoteId}/items", await h.Handle(new AddSupplierQuoteItemCommand(id, quoteId, r.InquiryItemId, r.Quantity, r.UnitPrice, r.DeliveryDate, r.TechnicalComplianceStatus, r.TechnicalNote), ct))))
            .RequirePermission(ApplicationPermissions.InquiryReceiveQuote);
        inquiries.MapPost("/{id:guid}/quotes/{quoteId:guid}/select", async (Guid id, Guid quoteId, SelectSupplierQuoteRequest r, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => { await h.Handle(new SelectSupplierQuoteCommand(id, quoteId, r.Reason), ct); return Results.NoContent(); })).RequirePermission(ApplicationPermissions.InquirySelectSupplier);
        inquiries.MapPost("/{id:guid}/quotes/{quoteId:guid}/reject", async (Guid id, Guid quoteId, RejectSupplierQuoteRequest r, InquiryCommandHandler h, CancellationToken ct) =>
            await Execute(async () => { await h.Handle(new RejectSupplierQuoteCommand(id, quoteId, r.Reason), ct); return Results.NoContent(); })).RequirePermission(ApplicationPermissions.InquiryReceiveQuote);
        return app;
    }
